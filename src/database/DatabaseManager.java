...
public class DatabaseManager {
	...
	private int createNewTables() throws SQLException {
		...
		connection.executeQuery(Constants.DB.createUserStateTable);
	...
	public boolean setPassword(Integer userId, int status, int messageId, String password) {
		...
		try {
			connection.initTransaction();
			final PreparedStatement preparedStatement = connection
					.getPreparedStatement("UPDATE Users SET status = ?, messageId = ?, password = ? WHERE userId = ?");
			preparedStatement.setInt(1, status);
			preparedStatement.setInt(2, messageId);
	...
	public String getUsername(Integer userId) {
		...
		try {
			final PreparedStatement preparedStatement = connection
					.getPreparedStatement("SELECT username FROM users WHERE userId = ?");
			preparedStatement.setInt(1, userId);
	...
	public String getPassword(Integer userId) {
		...
		try {
			final PreparedStatement preparedStatement = connection
					.getPreparedStatement("SELECT password FROM users WHERE userId = ?");
			preparedStatement.setInt(1, userId);
			final ResultSet result = preparedStatement.executeQuery();
			if (result.next()) {
	...
	public int getUserStatus(Integer userId) {
		...
		try {
			final PreparedStatement preparedStatement = connection
					.getPreparedStatement("SELECT status FROM users WHERE userId = ?");
			preparedStatement.setInt(1, userId);
			final ResultSet result = preparedStatement.executeQuery();
			if (result.next()) {
	...
	public int getUserMessageId(Integer userId) {
		...
		try {
			final PreparedStatement preparedStatement = connection
					.getPreparedStatement("SELECT messageId FROM users WHERE userId = ?");
			preparedStatement.setInt(1, userId);
			final ResultSet result = preparedStatement.executeQuery();
			if (result.next()) {
	...
	public boolean addUserState(Integer userId, Integer state) {
		int updatedRows = 0;
		try {
			connection.initTransaction();
			final PreparedStatement preparedStatement = connection
					.getPreparedStatement("INSERT INTO UserState (userId, state) VALUES (?, ?) ON DUPLICATE KEY UPDATE state = ?");
			preparedStatement.setInt(1, userId);
	...
	public boolean addUserState(Integer userId, Integer state, String json_courses) {
		int updatedRows = 0;
		try {
			connection.initTransaction();
			final PreparedStatement preparedStatement = connection
					.getPreparedStatement("INSERT INTO UserState (userId, state, courses) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE state = ?, courses = ?");
			preparedStatement.setInt(1, userId);
			preparedStatement.setInt(2, state);
			preparedStatement.setString(3, json_courses);
			preparedStatement.setInt(4, state);
			preparedStatement.setString(5, json_courses);
			updatedRows = preparedStatement.executeUpdate();
			connection.commitTransaction();
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public int getUserState(Integer userId) {
		...
		try {
			final PreparedStatement preparedStatement = connection
					.getPreparedStatement("SELECT state FROM UserState WHERE userId = ?");
			preparedStatement.setInt(1, userId);
			final ResultSet result = preparedStatement.executeQuery();
			if (result.next()) {
	...
	public String getUserCourses(Integer userId) {
		String courses = null;
		try {
			final PreparedStatement preparedStatement = connection
					.getPreparedStatement("SELECT courses FROM UserState WHERE userId = ?");
			preparedStatement.setInt(1, userId);
			final ResultSet result = preparedStatement.executeQuery();
			if (result.next()) {
